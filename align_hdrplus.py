...
def align_and_merge(images: Tensor,
    ...
    for i, comp_idx in enumerate(comp_idxs):
        ...
        if robust_merge:
            c_fft = torch.fft.fft2(comp_image_aligned, norm="forward")
            diff_fft = ref_fft - c_fft
            
            #Alpha and beta values :  0.014601984690114698 0.00044552966393963017
            #sample from a uniform gaussian with these heteroscadastic values
            #use the ref_image to sample the noise
            variances = ref_image * 0.014601984690114698 + 0.00044552966393963017
            #sample from a uniform gaussian with these heteroscadastic values
    ...
    if robust_merge:
        merged_image = torch.fft.ifft2(merged_image, norm="forward").abs()
    else:
